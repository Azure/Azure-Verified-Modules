...
Terraform can target Azure through multiple providers. **New AVM Terraform module authoring supports AzAPI only:** every resource, pattern, and utility module **MUST** use AzAPI for all Azure resource interactions. The AzureRM provider, `azurerm_*` resources, and `azurerm_*` data sources are prohibited throughout the repository, including examples and tests. AzAPI talks directly to the Azure Resource Manager REST API, providing day-zero access to new resource types, properties and API versions, built-in retry and pre-flight validation, and conceptual alignment with Bicep modules. See [AzAPI is mandatory for AVM Terraform modules]({{% siteparam base %}}/specs/tf/#azapi-is-mandatory-for-avm-terraform-modules) and [TFFR3]({{% siteparam base %}}/spec/TFFR3) for the complete requirement.
