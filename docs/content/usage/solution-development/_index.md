...
Terraform can target Azure through providers including AzAPI and AzureRM. **AVM Terraform module authoring supports AzAPI only:** every resource, pattern, and utility module **MUST** be built with AzAPI, and new AzureRM-based modules are not accepted. AzAPI talks directly to the Azure Resource Manager REST API, providing day-zero access to new resource types, properties and API versions, built-in retry and pre-flight validation, and conceptual alignment with Bicep modules. See [AzAPI is mandatory for AVM Terraform modules]({{% siteparam base %}}/specs/tf/#azapi-is-mandatory-for-avm-terraform-modules) for the full rationale. [TFFR3]({{% siteparam base %}}/spec/TFFR3) permits AzureRM only for a specific resource with no AzAPI equivalent; that exception never permits an AzureRM-based module.
