...
**Provider direction:** AVM Terraform modules from v1.0.0 onward **MUST** use the [AzAPI](https://registry.terraform.io/providers/Azure/azapi/latest) provider. The end-to-end lab in this guide currently demonstrates pre-v1.0.0 modules that are still published with the AzureRM provider; the underlying patterns (naming, composition, dependency wiring) remain the same when those modules graduate to AzAPI. See [Why AVM Terraform modules favor AzAPI]({{% siteparam base %}}/specs/tf/#why-avm-terraform-modules-favor-azapi) for the rationale.
{{% /notice %}}
