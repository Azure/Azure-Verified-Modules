...
| [TFFR5]({{% siteparam base %}}/spec/TFFR5) | Set `replace_triggers_refs` only when body paths require replacement. The non-empty static list **MUST** contain valid, unique JMESPath expressions; omit the argument when no paths are needed. `name` and `location` are already triggers, so don't repeat them. |
