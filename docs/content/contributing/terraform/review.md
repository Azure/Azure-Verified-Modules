...
- After [metadata maintenance]({{% siteparam base %}}/contributing/module-metadata/) is adopted, verify the module's root `metadata.json`, including all approved owners. Correct supported metadata through an engineering-owner-reviewed pull request to the module repository, not by editing Excel or CSV files.
- Confirm publication to the Terraform Registry and review the generated catalog publication that records the available module and its links. Link the module proposal to that publication so it is closed after the index update is merged and moves from "In Review" to "Done" in the [AVM - Module Triage](https://github.com/orgs/Azure/projects/529) project. During preview, coordinate the canonical index update with the AVM core team; preview CSVs do not replace it. Do not add an `Available` status field to metadata.
