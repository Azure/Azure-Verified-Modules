...
- After [metadata maintenance]({{% siteparam base %}}/contributing/module-metadata/) is adopted, verify the module's root `metadata.json`, including all approved owners. Correct supported metadata through a pull request to the module repository with approval from either metadata code-owner team, not by editing Excel or CSV files.
