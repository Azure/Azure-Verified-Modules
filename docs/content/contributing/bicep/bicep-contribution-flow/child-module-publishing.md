...
The Bicep Child Module Proposal issue primarily intends to cover the former, i.e. to publish a child module already existing in the [BRM (Bicep Registry Modules)](https://aka.ms/BRM) repository source code. However, the same issue allows also to request the development of the child module functionality, although the best way to address new functionality is to raise a feature request via the the [AVM Module issue](https://github.com/Azure/bicep-registry-modules/issues/new?template=avm_module_issue.yml).
...
1. Check the [Bicep Resource Modules index]({{% siteparam base %}}/indexes/bicep/bicep-resource-modules/#module-name-telemetry-id-prefix-github-teams-for-owners), or the underlying [module catalog JSON](https://github.com/Azure/Azure-Verified-Modules/blob/main/docs/static/module-indexes/v1/modules.json).
