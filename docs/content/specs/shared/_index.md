...
Preview API Provider versions **MAY** be used when:
...
- The resource/service/feature is GA but the only API version available for the GA resource/service/feature is a preview version
  ...
  - One can confirm what the latest GA version for a provider is on [Microsoft Learn](https://learn.microsoft.com/en-us/azure/templates/)

New services that are still in preview as well as any preview features on existing resource providers, **SHOULD NOT** be promoted and exposed, unless they are supported by the respective PG, and it's documented publicly.
