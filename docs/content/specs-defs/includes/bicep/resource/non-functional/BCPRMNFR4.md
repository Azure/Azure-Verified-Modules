...
Because each child-module is published on its own, you **must** ensure that each is registered in the MAR file at `microsoft/mcr: teams/bicep/bicep.yml` before the modules can be published. The MAR file can only be accessed by Microsoft FTEs.
