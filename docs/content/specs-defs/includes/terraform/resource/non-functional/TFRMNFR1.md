...
- All Terraform specifications ([TFFR]({{% siteparam base %}}/specs/tf/) and [TFNFR]({{% siteparam base %}}/specs/tf/) prefixed specs), including:
  - [TFFR3]({{% siteparam base %}}/spec/TFFR3) — AzAPI is mandatory for every module and submodule; the AzureRM provider, resources, and data sources are prohibited throughout the repository, including examples and tests.
