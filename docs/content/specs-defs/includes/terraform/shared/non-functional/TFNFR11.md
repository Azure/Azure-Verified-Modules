...
Sometimes we need to ensure that the resources created are compliant to some rules at a minimum extent, for example a `subnet` has to be connected to at least one `network_security_group`. The user **MAY** pass in a `security_group_id` and ask us to make a connection to an existing `security_group`, or want us to create a new security group.
