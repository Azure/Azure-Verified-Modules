...
Ideally there **SHOULD** be at least 2 module owners per module. After [metadata maintenance]({{% siteparam base %}}/contributing/module-metadata/) is adopted, each owner **MUST** be recorded in the root `metadata.json` file's `owners` array, with changes reviewed by the engineering owners. Until adoption, coordinate the ownership records in the [module indexes]({{% siteparam base %}}/indexes/) with the AVM core team. Record every owner, even when there are more than two; the compatibility CSV columns are not an ownership limit.
